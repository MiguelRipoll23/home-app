import { ElectricalEnergyMeasurementClient } from '@matter/main/behaviors/electrical-energy-measurement'
import { DescriptorClient } from '@matter/main/behaviors/descriptor'
import { Endpoint } from '@matter/main'
import { BaseDeviceControl } from './base-control'

const ENERGY_CLUSTER_ID = 0x91

export class ElectricalEnergyControl extends BaseDeviceControl {
  findEndpoint(endpointNumber?: number): Endpoint | undefined {
    const check = (ep: Endpoint) =>
      ep.maybeStateOf(ElectricalEnergyMeasurementClient) !== undefined || this.hasEnergyInDescriptor(ep)

    if (endpointNumber !== undefined) {
      const specific = this.findEndpointByNumber(endpointNumber)
      if (specific && check(specific)) return specific
      return undefined
    }

    for (const ep of this.node.endpoints) {
      if (Number(ep.number) === 0) continue
      if (check(ep)) return ep
    }
    return undefined
  }

  getCumulativeEnergy(): number | undefined {
    const ep = this.findEndpoint()
    if (!ep) return undefined
    try {
      const state = ep.maybeStateOf(ElectricalEnergyMeasurementClient)
      if (!state) return undefined
      const struct = state.cumulativeEnergyImported
      if (!struct || struct.energy === null) return undefined
      return struct.energy / 1000000
    } catch {
      return undefined
    }
  }

  private hasEnergyInDescriptor(endpoint: Endpoint): boolean {
    try {
      const desc = endpoint.maybeStateOf(DescriptorClient)
      if (!desc?.serverList) return false
      return desc.serverList.some((id: number) => id === ENERGY_CLUSTER_ID)
    } catch { return false }
  }
}
